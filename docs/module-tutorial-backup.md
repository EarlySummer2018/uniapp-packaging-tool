# DCloud UniApp 离线 SDK 模块配置完整教程

> **文档说明**：本文档基于 DCloud 官方离线 SDK 文档整理，涵盖 Android、iOS、鸿蒙三大平台的全部模块配置教程。
>
> **生成时间**：2026-05-29
>
> **原始文档来源**：https://nativesupport.dcloud.net.cn/AppDocs/usemodule/

---

## 目录

- [第一部分：Android 模块配置](#第一部分android-模块配置)
...
- [第二部分：iOS 模块配置](#第二部分ios-模块配置)
- [第三部分：鸿蒙模块配置](#第三部分鸿蒙模块配置)
- [第四部分：各平台注意事项](#第四部分各平台注意事项)
...
> **注意**：3.7.6开始不再提供"amap-libs-release.aar"文件，改为gradle集成！`geolocation-amap-release.aar`还需要继续添加到项目中。
...
> 应用的app id/app key等信息，从开发者后台->unipush->配置管理->应用管理界面查看。**注意：HBuilderX3.1.15之后需要添加GETUI_APPID属性。**
...
```xml
<!--小米厂商配置——开始-->
<meta-data android:name="MIPUSH_APPID" android:value="XM_${XIAOMI_APP_ID}" />
<meta-data android:name="MIPUSH_APPKEY" android:value="XM_${XIAOMI_APP_KEY}" />
<!--小米厂商配置——结束-->

<!--魅族厂商配置——开始-->
<meta-data android:name="MEIZUPUSH_APPID" android:value="MZ_${MEIZU_APP_ID}" />
<meta-data android:name="MEIZUPUSH_APPKEY" android:value="MZ_${MEIZU_APP_KEY}" />
<!--魅族厂商配置——结束-->
...
<!--OPPO厂商配置——开始-->
<meta-data android:name="OPPOPUSH_APPKEY" android:value="OP_${OPPO_APP_KEY}" />
<meta-data android:name="OPPOPUSH_APPSECRET" android:value="OP_${OPPO_APP_SECRET}" />
<!--OPPO厂商配置——结束-->

<!--VIVO厂商配置——开始-->
<meta-data android:name="com.vivo.push.app_id" android:value="${VIVO_APP_ID}" />
<meta-data android:name="com.vivo.push.api_key" android:value="${VIVO_APP_KEY}" />
<!--VIVO厂商配置——结束--> 
```
...
当添加了push-3.0.0.aar库时，在app目录下的build.gradle内添加：
...
> ⚠️ 该页面当前返回502错误，暂时无法获取内容。请直接访问原链接查看：
> https://nativesupport.dcloud.net.cn/AppDocs/usemodule/androidModuleConfig/share.html
...
> **注意：HBuilderX 3.99及以上版本，个推sdk由aar导入改为仓储方式。**

#### 需要拷贝的文件

| 路径 | 文件 |
|---|---|
| SDK\libs | `gtc.aar`, `gysdk-3.0.6.0.aar`, `oauth-univerify-release.aar` |
...
**HBuilderX 3.99及以上版本需增加仓储依赖**（先在项目根目录build.gradle添加个推仓库源）：
...
> 3.6.7以下版本还需要 `qq_mta-sdk-1.6.2.jar`
...
> **注意**：3.7.6开始不再提供"amap-libs-release.aar"文件，改为gradle集成。"weex_amap-release.aar"或"map-amap-release.aar"需要继续集成到项目中。
...
> **注意：3.7.6及以上版本，支付宝SDK改为gradle依赖，需移除alipaysdk-android-15.8.11.aar**
...
> **注意：3.7.6及以上版本，微信SDK改为gradle依赖，需移除wechat-sdk-android-without-mta-X.X.X.aar**
...
| 路径 | 文件 |
|---|---|
| SDK\libs | `statistic-release.aar`, `statistic-umeng-release.aar`, `umeng-abtest-v1.0.1.aar`, `utdid4all-XXX-proguard.jar` |

> - 3.3.6及以下版本用`alipayutdid.jar`代替`utdid4all-XXX-proguard.jar`
> - 3.8.3及以上版本，友盟SDK改为gradle依赖，需移除`umeng-abtest-v1.0.1.aar`、`utdid4all-XXX-proguard.jar`
...
> ⚠️ 该页面当前重定向到了 uni 小程序SDK首页，原始实人认证文档可能已被迁移或更新。
> 原链接：https://nativesupport.dcloud.net.cn/AppDocs/usemodule/androidModuleConfig/faceRecognitionVerify.html
...
| 路径 | 文件名 |
|---|---|
| SDK/libs | `ads-release.aar`, `ads-sigmob-release.aar`, `windAd.aar`, `wind-common.aar`(3.5.2及以上) |
...
| SDK | 版本 | HBuilderX最低版本 | 备注 |
|---|---|---|---|
| androidx | V1.1.0 | V3.8.4 | androidx相关依赖库 |
| fastjson | v1.2.83 | V3.6.17 | JSON解析库 |
| android-gif-drawable | V1.2.23 | V3.2.5 | gif图显示 |
| 移动安全联盟OAID | V1.0.25 | V3.2.5 | oaid获取 |
| glide | V4.9.0 | V3.2.5 | 图片预览 |
| fresco | V1.13.0 | V3.2.5 | nvue图片展示 |
| webkit | V1.3.0 | V3.6.15 | 暗黑模式支持 |

### 其他功能模块依赖库

| SDK | 版本 | HBuilderX最低版本 | 使用模块 |
|---|---|---|---|
| 个推push | V3.3.7.0 | V3.3.1 | unipush |
| 百度定位 | V7.5.0 | V2.0.0 | 定位 |
| 百度地图 | V5.4.1 | V2.0.0 | map |
| 高德定位 | V6.4.5 | V4.18 | 定位 |
| 高德地图 | V10.0.700 | V4.18 | map |
| 微信 | V6.8.0 | V3.7.12 | 登录/分享/支付 |
| 新浪微博 | V12.5.0 | V3.6.1 | 登录/分享 |
| QQ | V3.5.12 | V3.5.5 | 登录/分享 |
| 友盟统计 | V9.6.1 | V3.8.3 | 统计 |
| 百度语音 | V3.4.1.101 | V3.0.1 | 语音 |
| LiteAVSDK | V6.3.7089 | V3.0.1 | livepusher |
| 腾讯x5内核 | V4.3.0.1148_43697 | V3.0.7 | X5 |
| hms | V6.13.0.301 | V3.6.1 | 华为push |
| agcp | V1.9.1.301 | V3.6.1 | 华为AGC |
| 穿山甲&GroMore | V5.7.0.5 | V3.98 | 广告 |
| 优量汇广告 | V4.542.1412 | V3.93 | 广告 |
| 快手广告联盟 | V3.3.53.3 | V3.93 | 广告 |
| 快手内容联盟 | V3.3.53 | V3.93 | 广告 |
| sigmob广告 | V4.12.7 | V3.93 | 广告 |
| 百度广告 | V9.322 | V3.93 | 广告 |
| 华为广告 | V13.4.66.300 | V3.93 | 广告 |
| Pangle广告 | V5.0.0.3 | V3.93 | 广告 |
| google AdMob | V21.4.0 | V3.93 | 广告 |
| ijkplayer | V0.8.8 | V3.0.1 | 视频播放 |
| DanmakuFlameMaster | V0.6.2 | V2.0.0 | 弹幕 |
| lame | V3.100 | V3.2.16 | 音频录音(MP3) |
| play-services-auth | V19.2.0 | V3.2.7 | Google登录 |
| facebook-android-sdk | V16.1.3 | V3.91 | Facebook登录 |

---

# 第二部分：iOS 模块配置

> ⚠️ **注意**：以下 iOS 模块文档由 Task 子代理整理返回，由于篇幅限制，此处仅包含核心摘要信息。完整详细配置请参考原始链接或查看 Task 返回的完整结果。
...
> 💡 **建议**：对于 Geolocation 和 Payment 这两个无法访问的模块，请直接访问以下链接查看最新配置：
...
在`oh-package.json5`文件内`dependencies`字段下添加如下内容：
...
目前在鸿蒙端uni-app内置腾讯地图模块（使用web端方案），因此无需配置模块依赖。
...
> **注意**
> - 由于腾讯地图key在web端方案内使用，因此在申请腾讯地图key时需要将域名白名单留空以便地图能正确加载出来。
...
支付宝支付依赖此ohpm包：`@uni_modules/uni-payment-alipay@1.0.1`。
...
实人认证依赖此ohpm包：`@uni_modules/uni-facialrecognitionverify@1.0.2`。
...
### 2. HBuilderX 4.44版，离线打包白屏或者无法进行页面跳转

HBuilderX 4.44起需要升级webview的版本到1.5.0版本，build.gradle内配置参考：

```gradle
implementation 'androidx.webkit:webkit:1.5.0'
```

### 3. HBuilderX 4.41版，离线打包报 error: style attribute 'android:attr/windowOptOutEdgeToEdgeEnforcement' not found
...
### 4. 当设置targetSdkVersion为34时，上架 Google Play 应用白屏
...
### 5. 上架 Google Play 提示：SoLoader SDK 版本有问题
...
### 6. 离线SDK集成实人认证时报错：lib/*/libc++_shared.so
...
### 7. 解决上架 Google Play 审核报 DCloud SDK 包含从未知来源下载或安装应用的问题
...
### 8. 离线打包存在多个以uni-jsframework开头的文件
...
### 10. 升级到3.6.9，离线打包卡在Splash界面无法进入主页

3.6.9适配暗黑模式新增了依赖库，参考上述适配暗黑模式即可解决。

### 11. 升级离线SDK到3.5.0之后，离线打包报：java.lang.NoClassDefFoundError: Failed resolution of: Lcom/sample/breakpad/BreakpadInit;

离线SDK3.5.0及以上版本新增加了breakpad-build-release.aar，直接将这个库拷贝到libs目录下即可。详情可参考[文档](https://nativesupport.dcloud.net.cn/AppDocs/usesdk/android.html)。
...
### 14. 升级到3.2.5以上版本之后离线打包无法正常启动

**Java 8 配置**（3.2.5及以上版本支持了Java 8）：
...
**AndroidX 配置**（3.2.5版本之后更新到AndroidX，老项目需要在gradle.properties添加）：
...
其余AndroidX依赖库配置可参考[Android 原生工程配置](https://nativesupport.dcloud.net.cn/AppDocs/usesdk/android.html)

### 15. 应用启动白屏或者提示打包时未添加ui模块

大多数是因为build.gradle中配置了混淆。如果需要使用proguard混淆代码，需确保不要混淆SDK的代码。混淆配置和混淆文件可以参考SDK中的UniPlugin-Hello-AS项目。

### 16. 编译报错 style attribute 'android:attr/forceDarkAllowed' not found.
...
1. 将项目根目录下的build.gradle中的gradle插件版本升级到4.1.1：
...
### 21. 离线打包Android 10上无法启动相机

在application节点下添加provider节点（同第20条）：
...
**解决方式**：在 Xcode 中，进入 **TARGETS > Project Name > Build Settings > Build Options** 菜单，将 **Validate Workspace** 设置为 **Yes**。

### Q4: 升级Xcode15后编译报错提示文件重复添加，或运行时闪退
...
| 平台 | 成功抓取 | 失败/异常 | 总计 |
|------|---------|----------|------|
| **Android** | 13 个模块 | 2 个（Share、FaceRecognitionVerify 页面异常） | 15 个 |
| **iOS** | 12 个模块 | 2 个（Geolocation、Payment 502错误） | 14 个 |
| **鸿蒙** | 4 个模块 | 0 个 | 4 个 |
| **注意事项** | 2 个完整章节 | 0 个 | 2 个 |
| **总计** | **31 个** | **4 个** | **35 个** |
...
1. **适用场景**：本文档适用于使用 DCloud UniApp / 5+ App 进行**离线原生打包**的开发者
2. **版本要求**：不同配置项对 HBuilderX 和离线 SDK 版本有不同要求，请注意文档中的版本标注
3. **配置顺序**：建议按照文档顺序依次配置，避免遗漏依赖项
4. **常见问题**：遇到问题时优先查阅第四部分的注意事项章节
5. **官方资源**：
   - 离线 SDK 下载：https://nativesupport.dcloud.net.cn/AppDocs/usesdk/android.html
...
> 本文档内容整理自 DCloud 官方文档，仅供学习参考使用。
> 
> 由于官方文档可能随时更新，建议在实际开发时访问以下地址获取最新版本：
> - https://nativesupport.dcloud.net.cn/AppDocs/usemodule/
> 
> 如有版权问题或内容错误，请联系 DCloud 官方进行反馈。

---

**文档生成时间**：2026-05-29  
**生成工具**：AI Assistant (Powered by SOLO)  
**原始文档版本**：基于 DCloud 最新离线 SDK 文档
