...
fn get_face_recognition_template() -> ModuleTemplate {
    ModuleTemplate {
        module_name: "FacialRecognitionVerify".to_string(),
        description: "实人认证模块（DCloud/阿里云活体认证）".to_string(),
        android_config: AndroidModuleTemplate {
            required_aars: vec![
        ...
        ios_config: IosModuleTemplate {
            required_frameworks: vec![
                "DCUniBase.framework (Embed & Sign)".to_string(),
                "DCloudUTSFoundation.framework (Embed & Sign)".to_string(),
                "uniFacialRecognitionVerify.framework".to_string(),
                "AliyunFaceAuthFacade.framework".to_string(),
                "AliyunMobileRPC.framework".to_string(),
                "AliyunOSSiOS.framework".to_string(),
                "APBToygerFacade.framework".to_string(),
                "APPSecuritySDK.framework".to_string(),
                "BioAuthAPI.framework".to_string(),
                "BioAuthEngine.framework".to_string(),
                "deviceiOS.framework".to_string(),
                "DTFIdentityManager.framework".to_string(),
                "DTFSensorServices.framework".to_string(),
                "DTFUIModule.framework".to_string(),
                "DTFUtility.framework".to_string(),
                "MPRemoteLogging.framework".to_string(),
                "ToygerNative.framework".to_string(),
                "ToygerService.framework".to_string(),
                "CoreGraphics.framework".to_string(),
                "Accelerate.framework".to_string(),
                "SystemConfiguration.framework".to_string(),
                "AssetsLibrary.framework".to_string(),
                "CoreTelephony.framework".to_string(),
                "QuartzCore.framework".to_string(),
                "CoreFoundation.framework".to_string(),
                "CoreLocation.framework".to_string(),
                "ImageIO.framework".to_string(),
                "CoreMedia.framework".to_string(),
                "CoreMotion.framework".to_string(),
                "AVFoundation.framework".to_string(),
                "WebKit.framework".to_string(),
                "AudioToolbox.framework".to_string(),
                "CFNetwork.framework".to_string(),
                "MobileCoreServices.framework".to_string(),
                "AdSupport.framework".to_string(),
            ],
            required_libraries: vec![
                "libresolv.tbd".to_string(),
                "libz.tbd".to_string(),
                "libc++.tbd".to_string(),
                "libc++.1.tbd".to_string(),
                "libc++abi.tbd".to_string(),
                "libz.1.2.8.tbd".to_string(),
                "APBToygerFacade.bundle".to_string(),
                "BioAuthEngine.bundle".to_string(),
                "ToygerNative.bundle".to_string(),
            ],
            info_plist_keys: HashMap::from([(
                "NSCameraUsageDescription".to_string(),
                "我们需要使用摄像头进行人脸识别验证".to_string(),
            )]),
            url_schemes: vec![],
            plist_entry: "依赖 UTS 基础模块；构建时会 Embed & Sign DCUniBase.framework 与 DCloudUTSFoundation.framework，并移除 DCUniBase 已内置的重复链接库。".to_string(),
        },
    }
}
...
fn get_livepusher_template() -> ModuleTemplate {
    ModuleTemplate {
        ...
        description: "又拍云直播推流模块".to_string(),
        android_config: AndroidModuleTemplate {
            required_aars: vec![
        ...
        ios_config: IosModuleTemplate {
            required_frameworks: vec![
                "UPLiveSDKDll.framework (Embed & Sign，仅真机)".to_string(),
                "AVFoundation.framework".to_string(),
                "QuartzCore.framework".to_string(),
                "OpenGLES.framework".to_string(),
                "AudioToolbox.framework".to_string(),
                "VideoToolbox.framework".to_string(),
                "Accelerate.framework".to_string(),
                "CoreMedia.framework".to_string(),
                "CoreTelephony.framework".to_string(),
                "SystemConfiguration.framework".to_string(),
            ...
            required_libraries: vec![
                "liblibLivePush.a".to_string(),
                "libDCUniGPUImage.a".to_string(),
                "libDCUniLivePush.a (仅自定义组件模式 live-pusher)".to_string(),
                "libz.tbd".to_string(),
                "libbz2.tbd".to_string(),
                "libiconv.tbd".to_string(),
            ],
            info_plist_keys: HashMap::from([
                ("NSCameraUsageDescription".to_string(), "用于视频直播".to_string()),
                ("NSMicrophoneUsageDescription".to_string(), "用于音频采集".to_string()),
            ]),
            url_schemes: vec![],
            plist_entry: "iOS 又拍云直播推流模块：自动链接 liblibLivePush.a、libDCUniGPUImage.a 与 UPLiveSDKDll.framework；开启自定义组件模式时额外链接 libDCUniLivePush.a；UPLiveSDKDll.framework 会配置为 Embed & Sign，且仅支持真机。".to_string(),
        },
    }
}
